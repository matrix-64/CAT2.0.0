...
class CAT:
    ...
    def __init__(self,i_shape):
        ...
        self.k_img = cv2.resize(CAT.k_img, dsize = (int(self.image_shape[1]*0.9),int(self.image_shape[0]*0.5)), interpolation=cv2.INTER_AREA)
    ...
    def action(self,fings):
        ...
        if self.mode == "mouse":
            self.mmcat.action(self.shape,fings)

        if self.mode == "keyboard":
            #배경에 키보드 넣기
            h, w, c = self.k_img.shape
            roi = self.Bimage[int(self.image_shape[1]*0.085):int(self.image_shape[1]*0.085)+h, int(self.image_shape[0]*0.06):int(self.image_shape[0]*0.06)+w]
            ...
            self.Bimage[int(self.image_shape[1]*0.085):int(self.image_shape[1]*0.085)+h, int(self.image_shape[0]*0.06):int(self.image_shape[0]*0.06)+w] = dst
